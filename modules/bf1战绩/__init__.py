...
async def Bind(app: Ariadne, sender: Member, group: Group, message: MessageChain, player_name: RegexResult):
...
async def bg_reg(app: Ariadne, group: Group, message: MessageChain, qq: RegexResult, player_pid: RegexResult,
                 bg_num: RegexResult, bg_date: RegexResult):
...
async def bg_unrg(app: Ariadne, group: Group, message: MessageChain, player_pid: RegexResult):
...
async def bg_del(app: Ariadne, group: Group, sender: Member, message: MessageChain, bg_rank: RegexResult):
    # noinspection PyBroadException
    try:
...
async def bg_change(app: Ariadne, group: Group, sender: Member, message: MessageChain, bg_rank: RegexResult):
    # noinspection PyBroadException
    try:
        bg_rank = int(bg_rank.result.display)
        if bg_rank <= 0:
...
async def bg_check(app: Ariadne, group: Group, sender: Member, message: MessageChain):
    if not record.check_bind(sender.id):
        await app.send_message(group, MessageChain(
            f"请先使用'-绑定+你的游戏名字'进行绑定\n例如:-绑定shlsan13"
        ), quote=message[Source][0])
...
async def weapon(app: Ariadne, sender: Member, group: Group, message: MessageChain, player_name: RegexResult,
    ...
    try:
        ...
        logger.warning(e)
    # 武器计数器
    record.weapon_counter(sender.id, str(player_pid), str(player_name), str(weapon_type.result))
    return True
...
async def vehicle(app: Ariadne, sender: Member, group: Group, message: MessageChain, player_name: RegexResult,
    ...
    os.remove(SavePic)
    # 调用载具计数器
    record.vehicle_counter(sender.id, str(player_pid), player_name, str(vehicle_type.result))
    return True
...
async def player_stat_pic(app: Ariadne, sender: Member, group: Group, message: MessageChain, player_name: RegexResult):
    """
     TODO 4:生涯数据
    :param message:
    :param group:
...
async def recent(app: Ariadne, sender: Member, group: Group, message: MessageChain, player_name: RegexResult):
    if player_name.matched:
        # 判断玩家名字存不存在
        player_name = str(player_name.result).replace("+", "").replace(" ", "")
        # noinspection PyBroadException
...
async def matches(app: Ariadne, sender: Member, group: Group, message: MessageChain, player_name: RegexResult):
    if player_name.matched:
        # 判断玩家名字存不存在
        player_name = str(player_name.result).replace("+", "").replace(" ", "")
        # noinspection PyBroadException
    ...
    record.matches_counter(sender.id, str(player_pid), str(player_name))
    return True
...
async def player_tyc(app: Ariadne, sender: Member, group: Group, message: MessageChain, player_name: RegexResult):
    if player_name.matched:
        # 判断玩家名字存不存在
        player_name = str(player_name.result).replace("+", "").replace(" ", "")
        # noinspection PyBroadException
...
async def bf_status(app: Ariadne, group: Group, message: MessageChain):
    ...
    data_list2 = [
        ...
        data2["amounts"]["communitySoldierAmount"], data2["amounts"]["communityQueueAmount"],
        data2["amounts"]["communitySpectatorAmount"], data2["amounts"]["diceServerAmount"],
        data2["amounts"]["diceSoldierAmount"], data2["amounts"]["diceQueueAmount"],
        data2["amounts"]["diceSpectatorAmount"]
    ]
    await app.send_message(group, MessageChain(
        # "----亚服信息----\n",
        # "=" * 13, "\n",
        ...
        "=" * 13, "\n",
        "私服(官服):\n",
        f"服务器:{int(data_list2[4])}({int(data_list2[8])})\n",
        f"人数:{int(data_list2[5])}({int(data_list2[9])})\n",
        f"排队:{int(data_list2[6])}({int(data_list2[10])})\n",
        f"观众:{int(data_list2[7])}({int(data_list2[11])})\n",
        "=" * 13, "\n",
        "征服:%d" % data_list1[0] + "\t",
        "行动:%d" % data_list1[1] + "\n",
        "前线:%d" % data_list1[2] + "\t",
...
@channel.use(ListenerSchema(listening_events=[GroupMessage],
                            ...
                            inline_dispatchers=[
                                Twilight(
                                    [FullMatch("-查询统计").space(PRESERVE)]
                                )
                            ]))
async def bf_checkCounter(app: Ariadne, sender: Member, group: Group, message: MessageChain):
    if not record.check_bind(sender.id):
        await app.send_message(group, MessageChain(
            f"没有找到你的信息!\n请先使用'-绑定+你的游戏名字'进行绑定\n例如:-绑定shlsan13"
        ), quote=message[Source][0])
    ...
    try:
        ...
        return
    bind_info = record.get_bind_counter(sender.id)
    weapon_info = record.get_weapon_counter(sender.id)
    vehicle_info = record.get_vehicle_counter(sender.id)
    stat_info = record.get_stat_counter(sender.id)
    recent_info = record.get_recent_counter(sender.id)
    matches_info = record.get_matches_counter(sender.id)
    tyc_info = None
    report_info = None
    # noinspection PyBroadException
    try:
        tyc_info = record.get_tyc_counter(sender.id)
        report_info = record.get_report_counter(sender.id)
    except Exception as e:
        logger.warning(e)
        pass
    await app.send_message(group, MessageChain(
...
@channel.use(ListenerSchema(listening_events=[GroupMessage],
                            ...
                            inline_dispatchers=[
                                Twilight(
                                    [FullMatch("-help bf1战绩").space(PRESERVE)]
                                )
                            ]))
...
async def op_info(app: Ariadne, group: Group, message: MessageChain):
    global bf_aip_header, bf_aip_url
    session = record.get_session()
    bf_aip_header["X-Gatewaysession"] = session
    body = {
