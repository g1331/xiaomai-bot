...
async def search_server(app: Ariadne, group: Group, message: MessageChain, server_name: RegexResult):
...
async def get_server_detail(app: Ariadne, group: Group, message: MessageChain,
...
# 谁在玩功能
@channel.use(ListenerSchema(listening_events=[GroupMessage],
                            decorators=[Perm.require(16), Switch.require("bf1战绩"),
                                        DuoQ.require()],
                            inline_dispatchers=[
                                Twilight(
                                    [
...
async def get_server_playerList(app: Ariadne, group: Group, message: MessageChain,
    ...
    try:
        ...
        return False
    # 获取服务器id信息
    id_dict = await get_bfgroup_ids(app, group, message, server_rank)
...
async def get_server_playerList_pic(app: Ariadne, sender: Member, group: Group, message: MessageChain,
                                    server_rank: RegexResult):
    global client
    # 服务器序号检测
    try:
...
async def managerAccount_login(app: Ariadne, group: Union[Group, Friend], message: MessageChain,
                               account_pid: RegexResult, remid: RegexResult, sid: RegexResult):
...
async def bfgroup_search_log(app: Ariadne, sender: Member, group: Group, message: MessageChain,
    ...
    # 有匹配的时候如果绑定不一致就判断权限
        ...
        with open(file_path, 'r', encoding="utf-8") as file1:
            ...
            try:
                ...
                return False
        if bfgroups_name != group_name and Perm.get(sender, group) < 128:
            await app.send_message(group, MessageChain(
                f'你没有进行此操作的权限,你的权限级:{Perm.get(sender, group)},所需权限级:{128}'
            ), quote=message[Source][0])
            return False
...
async def get_bfgroup_session(app: Ariadne, group: Group, message: MessageChain, server_rank: int) -> str:
    """
...
async def get_bfgroup_ids(app: Ariadne, group: Group, message: MessageChain, server_rank: int) -> dict:
    """
    失败返回False，成功返回dict
    :param group:
    :param app:
...
async def kick(app: Ariadne, sender: Member, group: Group, message: MessageChain, action: RegexResult,
               server_rank: RegexResult, player_name: RegexResult, reason: RegexResult):
    # 服务器序号检测
    try:
        server_rank = int(str(server_rank.result)) - 1
        if server_rank < 0 or server_rank > 30:
...
async def kick_by_searched(app: Ariadne, sender: Member, group: Group, message: MessageChain,
                           action: RegexResult, player_name: RegexResult, reason: RegexResult):
    ...
    for key in result_dict:
        if result_dict[key]["player_matched"]:
            ...
            try:
                ...
                await app.send_message(group, MessageChain(
                    f'操作超时!已退出换图'), quote=message[Source][0])
                return
...
async def add_ban(app: Ariadne, sender: Member, group: Group, message: MessageChain, action: RegexResult,
                  server_rank: RegexResult, player_name: RegexResult, reason: RegexResult):
...
async def del_ban(app: Ariadne, sender: Member, group: Group, message: MessageChain, action: RegexResult,
                  server_rank: RegexResult, player_name: RegexResult):
...
async def get_vban_list(app: Ariadne, group: Group, message: MessageChain, vban_rank: RegexResult):
    ...
    try:
        # async with httpx.AsyncClient() as client:
        response = await client.get('https://manager-api.gametools.network/api/autoban', headers=headers, params=params)
    except:
        await app.send_message(group, MessageChain(
            "网络出错请稍后再试!"
        ), quote=message[Source][0])
...
async def move_player(app: Ariadne, sender: Member, group: Group, message: MessageChain, action: RegexResult,
                      server_rank: RegexResult, player_name: RegexResult, team_index: RegexResult):
    # 服务器序号检测
    try:
...
async def change_map(app: Ariadne, sender: Member, group: Group, message: MessageChain, action: RegexResult,
                     server_rank: RegexResult, map_index: RegexResult):
        ...
        if len(map_index_list) > 1:
            ...
            for item in map_index_list:
                map_index_list[i] = f"{i}#{item}●\n".replace("流血", "流\u200b血") if (
                        item.startswith('行動模式') and item.endswith(('聖康坦的傷痕', '窩瓦河',
                                                                   '海麗絲岬', '法歐堡', '攻佔托爾', '格拉巴山',
                                                                   '凡爾登高地', '加利西亞', '蘇瓦松', '流血宴廳',
                                                                   '澤布呂赫',
                                                                   '索姆河',
                                                                   '武普庫夫山口'))) else f"{i}#{item}\n".replace('流血',
                                                                                                            '流\u200b血')
                choices.append(str(i))
                i += 1
...
async def change_map_bylist(app: Ariadne, sender: Member, group: Group, message: MessageChain, action: RegexResult,
                            server_rank: RegexResult):
    # 服务器序号检测
    try:
        server_rank = int(str(server_rank.result)) - 1
...
async def add_vip(app: Ariadne, sender: Member, group: Group, message: MessageChain, action: RegexResult,
                  server_rank: RegexResult, player_name: RegexResult, days: RegexResult):
...
async def del_vip(app: Ariadne, sender: Member, group: Group, message: MessageChain, action: RegexResult,
                  server_rank: RegexResult, player_name: RegexResult):
