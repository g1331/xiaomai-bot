...
@dispatch(
    Twilight(
        [
            FullMatch("-公告"),
            ArgumentMatch("-t", "-time", optional=True, type=int, default=1)
            @ "time_interval",
...
async def push_handle(
    ...
    source: Source,
    member: Member,
    time_interval: ArgResult,
    function_name: RegexResult,
    content: RegexResult,
):
    ...
    if not (0 < time_interval < 10):
        return await app.send_message(
            group,
            MessageChain(f"间隔时间需要在0~10之间哦!匹配到的时间:{time_interval}"),
            quote=source,
        )
    ...
    # 获取打开了功能的群
    available_modules = (
        module_controller.get_required_modules()
        + module_controller.get_available_modules()
    )
    module_name = None
    for channel_temp in available_modules:
        temp_name = (
            module_controller.get_metadata_from_module_name(channel_temp).display_name
            or saya.channels[channel_temp].meta["name"]
            or channel_temp.split(".")[-1]
        )
    ...
    if not module_name:
        return await app.send_message(
            group,
            MessageChain(f"没有在运行插件中找到 {function_name} 哦~"),
            quote=source,
        )
    push_list = []
    for group_id in account_controller.account_dict:
        ...
        if (
            target_app
            and target_group
            and module_controller.if_module_switch_on(module_name, target_group)
        ):
            push_list.append([target_app, target_group])
    if not push_list:
        return await app.send_message(
            group, MessageChain("没有满足条件的群哦~"), quote=source
        )
    try:
...
async def pusher(push_list, content, time):
    for item in push_list:
        ...
        try:
            ...
            logger.error(f"推送公告到群{target_group.id}时发送错误!{e}")
        finally:
