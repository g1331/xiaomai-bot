...
async def helper(app: Ariadne, group: Group, source: Source):
    ...
    for i, channel_temp in enumerate(unavailable_module_list):
        module_columns.append(ColumnList(rows=[
            ColumnListItem(
                # 副标题
                subtitle=f"{i + len(normal_module_list) + len(required_module_list)}.{module_controller.get_metadata_from_module_name(channel_temp).display_name or channel_temp.split('.')[-1]}",
