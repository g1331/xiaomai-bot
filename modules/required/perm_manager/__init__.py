...
async def change_user_perm(
    ...
    for target in targets:
        if await Permission.get_user_perm(event) < (
            ...
            await Permission.get_user_perm_byID(target_group.id, target)
            == Permission.BotAdmin
        ):
            ...
            await orm.insert_or_update(
                ...
                data={"group_id": target_group.id, "qq": target, "perm": perm},
            )
...
async def change_group_perm(
    app: Ariadne, group: Group, group_id: RegexResult, perm: RegexResult, source: Source
):
    group_id = int(group_id.result.display) if group_id.matched else group.id
    try:
        perm = int(perm.result.display)
    ...
    if target_group.id == config.test_group:
        return await app.send_message(
            group,
            MessageChain(f"无法通过该指令修改测试群({target_group.id})权限!"),
            quote=source,
        )
    await orm.insert_or_update(
        GroupPerm,
    ...
    return await app.send_message(
        group,
        MessageChain(f"已修改群{target_group.name}({target_group.id})权限为{perm}"),
        quote=source,
    )
...
async def change_group_perm_type(
    ...
    await orm.insert_or_update(
        ...
        condition=[GroupSetting.group_id == target_group.id],
    )
    if permission_type == "admin":
        for member in await target_app.get_member_list(target_group):
            if (
                await Permission.get_user_perm_byID(target_group.id, member.id)
        ...
        for member in await target_app.get_member_list(group):
            ...
            if now_perm != target_perm:
                await orm.insert_or_update(
                    table=MemberPerm,
                    data={"qq": member.id, "group_id": group.id, "perm": target_perm},
                    condition=[
                        MemberPerm.qq == member.id,
                        MemberPerm.group_id == group.id,
                    ],
...
async def get_vg_list(app: Ariadne, group: Group, source: Source):
    vip_group_list = []
    ...
    if not vip_group_list:
        return await app.send_message(
            group, MessageChain("当前没有VIP群~"), quote=source
        )
    ...
    vg_group_list_column = [
        Column(elements=vg_group_list_column[i : i + 20])
        for i in range(0, len(vg_group_list_column), 20)
    ]
    return await app.send_message(
        group,
        MessageChain(
...
async def get_perm_list(
    ...
    if group_id != group.id:
        ...
        if group_id not in account_controller.total_groups:
            return await app.send_message(
                event.sender.group, MessageChain(f"没有找到群{group_id}~"), quote=source
            )
        target_app, target_group = await account_controller.get_app_from_total_groups(
            group_id
        )
    ...
    for member in member_list:
        if (
            member.id not in perm_dict
            and Permission.member_permStr_dict[member.permission.name] != 16
        ):
            perm_dict[member.id] = Permission.member_permStr_dict[
    ...
    perm_list_column = [
        Column(elements=perm_list_column[i : i + 20])
        for i in range(0, len(perm_list_column), 20)
    ]
    return await app.send_message(
        group,
        MessageChain(
...
async def change_globalBlack(
    ...
    for target in targets:
        if action == "添加":
            if (
                target in await Permission.get_BotAdminsList()
                or target == config.Master
            ):
                error_targets.append((target, "无法修改BOT管理/Master的权限哦~"))
                continue
            if target in global_black_list:
                ...
                await orm.insert_or_update(
                    table=MemberPerm,
                    data={"qq": target, "group_id": 0, "perm": -1},
                    condition=[MemberPerm.qq == target, MemberPerm.group_id == 0],
                )
        else:
...
async def get_globalBlack_list(app: Ariadne, group: Group, source: Source):
    ...
    if len(global_black_list) == 0:
        return await app.send_message(
            group, MessageChain("全局黑名单为空哦~"), quote=source
        )
...
async def get_botAdmins_list(app: Ariadne, group: Group, source: Source):
    ...
    if len(admin_list) == 0:
        return await app.send_message(
            group, MessageChain("当前还没有BOT管理哦~"), quote=source
        )
...
async def auto_del_perm(app: Ariadne, group: Group, member: Member):
    ...
    await orm.delete(
        table=MemberPerm,
        condition=[MemberPerm.qq == member.id, MemberPerm.group_id == group.id],
    )
    if Permission.GroupOwner >= target_perm >= Permission.GroupAdmin:
        return await app.send_message(
            group, f"已自动删除退群成员{member.name}({member.id})的权限"
        )


# 自动添加管理群的权限/Master/admins
@listen(MemberJoinEvent)
async def auto_add_perm(
    app: Ariadne, group: Group, member: Member, event: MemberJoinEvent
):
    if app.account != await account_controller.get_response_account(group.id):
        return
    ...
    if permission_type == "admin":
        ...
        await app.send_message(
            group, f"已自动修改成员{member.name}({member.id})的权限为32"
        )
...
async def auto_change_admin_perm(app: Ariadne, event: MemberPermissionChangeEvent):
    if app.account != await account_controller.get_response_account(
