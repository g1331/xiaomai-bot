...
from graia.ariadne.message.parser.twilight import (
    Twilight, FullMatch, SpacePolicy, RegexMatch, ArgumentMatch, WildcardMatch
)
...
class Tarot:
    ...
    async def get_daily_fortune(user_id: int, group_id: int) -> MessageChain:
        ...
        if await Tarot._check_daily_limit(user_id, group_id, "daily_fortune"):
            return MessageChain([Plain("你今天已经进行过每日运势占卜了，请明天再来吧～")])
        ...
        # 记录占卜
        await Tarot._record_divination(user_id, group_id, "daily_fortune", [card], [position])
    ...
    async def get_three_card_spread(user_id: int, group_id: int) -> MessageChain:
        ...
        if await Tarot._check_hourly_limit(user_id, group_id, "three_card"):
            return MessageChain([Plain("你在一小时内已经进行过三张牌阵占卜了，请稍后再试～")])
    ...
    async def get_celtic_cross(user_id: int, group_id: int) -> MessageChain:
        ...
        if await Tarot._check_daily_limit(user_id, group_id, "celtic_cross"):
            return MessageChain([Plain("你今天已经进行过凯尔特十字占卜了，请明天再来吧～")])
...
带着具体问题进行占卜，保持开放和诚实的心态。塔罗牌是自我反思的工具，而非绝对的预言。"""
    ...
    @staticmethod
    async def _record_divination(user_id: int, group_id: int, divination_type: str, 
                               cards: List[Dict], positions: List[str]):
        """记录占卜历史"""
    ...
    @staticmethod
    async def _find_card_by_name(name: str) -> Optional[Dict]:
        """根据牌名查找塔罗牌"""
        data = Tarot._load_tarot_data()
        for kind in ["major", "pentacles", "wands", "cups", "swords"]:
...
"""
    ...
    @staticmethod
    def _get_holiday_info() -> Optional[str]:
        """获取当前日期的节日信息"""
        now = datetime.now()
