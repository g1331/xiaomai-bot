...
class TestBlazeAutoReconnect(unittest.TestCase):
    ...
    async def test_auto_reconnect_on_connection_error(
        self, mock_blaze_data, mock_client_manager, mock_bf1da
    ):
        """测试连接错误时的自动重连"""
    ...
    @patch("utils.bf1.bf_utils.BlazeClientManagerInstance")
    @patch("utils.bf1.bf_utils.BlazeData")
    async def test_auto_reconnect_on_send_exception(
        self, mock_blaze_data, mock_client_manager, mock_bf1da
    ):
        """测试发送异常时的自动重连"""
        ...
        mock_socket_fail.send = AsyncMock(
    ...
    def test_blaze_query_response_functionality(self):
        ...
        # 测试成功响应
        success_response = BlazeQueryResponse(
            BlazeQueryResult.SUCCESS, data={"test": "data"}
        )
        ...
        # 测试超时响应
        timeout_response = BlazeQueryResponse(
            BlazeQueryResult.TIMEOUT, error_message="超时错误"
        )
